...
class PoseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            rgb = np.array(Image.open(self.rgb_files[idx]))/255.0 
            depth = np.array(Image.open(self.depth_files[idx]))/1000.0
            label = np.array(Image.open(self.label_files[idx]))
            meta = self.load_pickle(self.meta_files[idx])
